...
public class DatabaseServiceBadSerializable implements Serializable {

	// Статична змінна для реалізації шаблону Singleton.
	private static final DatabaseServiceBadSerializable INSTANCE = new DatabaseServiceBadSerializable();
	...
	private DatabaseServiceBadSerializable() {
		users = new ArrayList<>();
	}

	// Метод для отримання екземпляра сервісу (Singleton).
	public static DatabaseServiceBadSerializable getInstance() {
		return INSTANCE;
	}
